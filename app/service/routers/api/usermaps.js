...
var oModel= require('../../models/core')('UserMaps');
...
router.get('/:module', checkIsAdmin, function(req, res, next){
  ...
  if(module){
    oModel.find({'module': module}, handler, res);
  }else{
    res.send({
      status: 'error',
...
router.delete('/', checkIsAdmin, function(req, res, next){
  ...
  oModel.remove({'_id': objid}, function(){
    oModel.find({'module': req.query.module}, handler, res);
  }, res);
});
